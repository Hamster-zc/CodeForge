...
CodeForge v0.1.2 是一个轻量级 Coding Agent 编排框架。它不直接调用模型 API，
...
Architect 完成后，终端会展示方案并询问 `Continue? [y/n]`。v0.1.2 不提供自动
...
- v0.1.2 没有长期记忆、向量数据库、自动训练或模型 API 客户端。
